...
\noindent for all $\that \in \Nteger_{\StPls}$, and where $\delta_{\StPls,\that}$ is the Kronecker delta function (the abstract state $\StPls$ of $\Mrkv_{f}$ is absorbing) 
  ...
  % Suppose we have an equiprobable discretized representation of the distribution of $\pLev$.  Specifically, we have a finite Markov chain $\Mrkv_{f}$ with $\StNum + 1$ discrete states in the set $\Nteger_{\StPls} \doteq \{1,2,\dots,\StNum,\StPls\}$ 
