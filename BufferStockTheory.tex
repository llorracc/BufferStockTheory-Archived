...
The paper's main technical contributions are to articulate the (surprisingly loose) conditions under which the problem defines a contraction mapping with a nondegenerate consumption function, and conditions under which the resulting consumption function a implies expstence of a `target' wealth-to-permanent-income ratio.  (This is the sense in which the paper studies the class of `buffer stock' saving models.)  The key condition required for target saving is that the consumer's parameter values need to satisfy a ``Growth Impatience Condition'' which relates preference parameters to the growth rate of income.
