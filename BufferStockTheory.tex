...
\begin{abstract}
  This paper builds theoretical foundations for rigorous understanding of `buffer stock' saving models, and pairs each theoretical result with a quantitative exploration.  After articulating conditions under which the consumption function converges, the paper shows that the `target' saving behavior that defines buffer stock saving arises only under conditions strictly stronger than those that guarantee convergence of the consumption (and value) function.  It also shows that average consumption growth equals average income growth in a small open economy populated by buffer stock savers.  Together, the (provided) numerical tools and analytical results compose a comprehensive toolkit for understanding buffer stock models.
\end{abstract}
